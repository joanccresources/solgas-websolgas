...
export default async function Action() {
  ...
  const section_fields = data?.page?.section_rels?.find(
    (item: { name: string }) => item?.name === "Banner consejos seguridad"
  )?.content_rels;
  return (
    <ActionSection hasGradient={false} heightClassName="h-[140px] z-30">
        ...
        <div>
          <Link href={section_fields?.enlace_boton?.value} target="_blank">
            <Button
              ...
              border={"primary"}
              color={"white"}
              className="lg:text-base sm:text-sm text-[12px] font-clan-pro-bold lg:min-w-[180px] min-w-[130px] lg:h-[50px] h-[36px]"
            >
